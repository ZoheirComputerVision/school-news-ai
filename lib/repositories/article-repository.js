...
class ArticleRepository extends BaseRepository {
  ...
  getStats() {
    ...
    return {
      ...
      byCategory: {
        news: all.filter(c => c.category === 'news' && c.status === 'published').length,
        activity: all.filter(c => c.category === 'activity' && c.status === 'published').length,
        announcement: all.filter(c => c.category === 'announcement' && c.status === 'published').length,
      },
    };
  }
