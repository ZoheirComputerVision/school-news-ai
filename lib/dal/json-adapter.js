...
class JsonAdapter extends DataAdapter {
  ...
  init() {
    ...
    this.tables = {
      ...
      editorial_items: this._load('editorial_items'),
      editorial_audit: this._load('editorial_audit'),
      advertisers: this._load('advertisers'),
      campaigns: this._load('campaigns'),
      ad_events: this._load('ad_events'),
      tenants: this._load('tenants'),
      tenant_config: this._load('tenant_config'),
      tenant_settings: this._load('tenant_settings'),
      tenant_users: this._load('tenant_users'),
      tenant_pages: this._load('tenant_pages'),
      plans: this._load('plans'),
      subscriptions: this._load('subscriptions'),
      invoices: this._load('invoices'),
      usage_metrics: this._load('usage_metrics'),
      payment_events: this._load('payment_events'),
    };
    this._seedDefaults();
  ...
  _seedDefaults() {
    if (this.tables.sources.length === 0) {
      this.tables.sources = [
        ...
        { id: 2, name: 'مصادر حكومية', url: config.AI.MINISTRY_URL, type: 'web', is_active: 1, trust_score: 0.9, last_scraped: null, created_at: new Date().toISOString() },
        { id: 3, name: 'إدخال يدوي', url: '', type: 'manual', is_active: 1, trust_score: 1.0, last_scraped: null, created_at: new Date().toISOString() },
      ];
      this._save('sources');
    }
