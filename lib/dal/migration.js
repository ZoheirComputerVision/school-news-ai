  ...
  for (const table of TABLES) {
    ...
    try {
      ...
      for (const record of records) {
        try {
          if (table === 'settings') {
            const existing = sqlite.find('settings', (r) => r.key === record.key);
            if (existing.length > 0) {
              sqlite.update('settings', existing[0].id, record);
            } else {
              sqlite.create('settings', record);
            }
          } else {
          ...
          report.tables[table].errors.push({ id: record.id || record.key, error: err.message });
...
async function verifyMigration() {
  ...
  const summary = TABLES.map(t => ({ table: t, json: json.count(t), sqlite: sqlite.count(t) }));
  sqlite.close();
  return { consistent: discrepancies.length === 0, discrepancies, summary };
}
