...
class SqliteAdapter extends DataAdapter {
  ...
  _seedDefaults() {
    ...
    if (sourceCount === 0) {
      ...
      insert.run('web-ministry', 'وزارة التربية الوطنية', config.AI.MINISTRY_URL, 'web', 'الجزائر', '', 'official', 'active', 1, 0.9, 60, now);
      insert.run('web-douane', 'مديرية التربية لولاية تيارت', 'https://www.douane.gov.dz/', 'web', 'تيارت', '', 'official', 'active', 1, 0.85, 60, now);
      insert.run('manual-admin', 'إدخال يدوي - إدارة الثانوية', '', 'manual', 'تيارت', 'عين كرمس', 'internal', 'active', 1, 1.0, 0, now);
    }
